...
export class AuthManager {
  ...
  async sendMagicLink(email) {
    try {
      ...
      // In development/test mode, also return the magic link URL for easy testing
      if (
        process.env.NODE_ENV === "development" ||
        process.env.NODE_ENV === "test"
      ) {
        const loginUrl = `${process.env.APP_URL}/api/auth/verify?token=${token}`;
        response.magicLinkUrl = loginUrl;
      }
  ...
  async validateSession(sessionId) {
    try {
      ...
      const session = await this.db.sessions.getValidById(sessionId);
  ...
  async cleanupExpired() {
    try {
      ...
      logger.debug("🧹 Cleanup expired tokens and sessions (placeholder)");
