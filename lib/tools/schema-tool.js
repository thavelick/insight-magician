...
import { BaseTool } from "../tool-executor.js";
...
export class SchemaTool extends BaseTool {
  ...
  async execute(parameters, context) {
    try {
      ...
      try {
        ...
        // Return all tables
        const tableCount = Object.keys(fullSchema).length;
        const totalRows = Object.values(fullSchema).reduce(
        ...
        return {
          ...
          data: {
            schema: fullSchema,
            summary: this._formatFullSchemaSummary(
              fullSchema,
              tableCount,
              totalRows,
            ),
      ...
      console.error("Schema tool execution failed:", error);
  ...
  /**
   * Format a summary for the full database schema
   */
  _formatFullSchemaSummary(schema, tableCount, totalRows) {
    if (tableCount === 0) {
    ...
    const tableList =
      tableNames.slice(0, 5).join(", ") +
      (tableNames.length > 5 ? ` and ${tableNames.length - 5} more` : "");

    return (
      `Database contains ${tableCount} table(s) with ${totalRows} total rows. ` +
      `Tables: ${tableList}.`
    );
  }
}
