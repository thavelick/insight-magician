...
export class ListWidgetsTool extends BaseTool {
  ...
  getPromptDescription() {
    return "Get information about current widgets on the dashboard including their titles, types, queries, dimensions, and status";
  }

  getUsageGuidance() {
    return "Use list_widgets when users ask about their current dashboard, widgets, or what visualizations they have";
  }

  getExampleQueries() {
    return [
      "What widgets do I have?",
      "Show me my current dashboard",
      "What visualizations am I working with?",
      "Tell me about my widgets",
    ];
  }
  ...
  async execute(parameters, context) {
    try {
      logger.debug("🔧 ListWidgetsTool: Getting current widget state");
      ...
      if (context?.widgets && Array.isArray(context.widgets)) {
        ...
        logger.debug("🔧 Using widgets from context");
      } else {
        widgetData = this.getWidgetDataFromStorage();
        logger.debug("🔧 Using widgets from sessionStorage");
      }
      ...
      logger.error("❌ ListWidgetsTool execution failed:", error);
      return {
        success: false,
  ...
  getWidgetDataFromStorage() {
    try {
      ...
      logger.warn("Failed to parse widget data from sessionStorage:", error);
      return [];
    }
  }
