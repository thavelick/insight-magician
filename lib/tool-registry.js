...
import { WidgetEditTool } from "./tools/widget-edit-tool.js";
...
class ToolRegistry {
  ...
  registerDefaultTools() {
    ...
    this.registerTool(new WidgetEditTool());
  }
  ...
  generateSystemPrompt() {
    ...
    for (const tool of tools) {
      ...
      // Collect technical usage examples from all tools
      const toolExamples = tool.getToolUseExamples();
      if (toolExamples?.trim()) {
        toolUseExamples += `${toolExamples}\n\n`;
      }
    }
...
${toolUseExamples ? `**Technical Documentation:**\n${toolUseExamples}` : ""}
