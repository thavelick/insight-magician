...
test("ChatAPI formatChatHistoryForAPI formats messages correctly", () => {
  ...
  const messages = [
    { role: "user", content: "Hello", timestamp: "2023-01-01T00:00:00Z", id: "1" },
    { role: "assistant", content: "Hi there!", timestamp: "2023-01-01T00:00:01Z", id: "2" },
  ];
...
test("ChatAPI sendMessage calls API with correct parameters", async () => {
  ...
  const chatHistory = [
    { role: "user", content: "Previous message", timestamp: "2023-01-01T00:00:00Z", id: "1" },
  ];
...
test("ChatAPI sendMessage handles missing error message", async () => {
  ...
  await expect(chatAPI.sendMessage("Test", [])).rejects.toThrow("Failed to get AI response");
});
