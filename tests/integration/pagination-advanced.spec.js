...
test.describe("Pagination Edge Cases", () => {
  ...
  test("should handle different page sizes within limits", async ({ page }) => {
    ...
    const minResponse = await page.request.post("/api/query", {
      data: {
    ...
    const maxResponse = await page.request.post("/api/query", {
      data: {
        filename: uploadedFilename,
        query: "SELECT * FROM users",
        page: 1,
  ...
  test("should fallback to counting all results when COUNT query fails", async ({ page }) => {
    const { uploadedFilename } = await setupDatabase(page);
  ...
  test("should calculate pagination metadata correctly for edge cases", async ({ page }) => {
    const { uploadedFilename } = await setupDatabase(page);

    // Test requesting page beyond available data
    const beyondResponse = await page.request.post("/api/query", {
      data: {
        filename: uploadedFilename,
        query: "SELECT * FROM users",
        page: 99,
    ...
    const exactResponse = await page.request.post("/api/query", {
      data: {
        ...
        query: "SELECT * FROM users",
        page: 2,
        pageSize: 5, // 10 total rows / 5 = 2 pages exactly
      },
    });
