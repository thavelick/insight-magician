...
test.describe("AI Chat Real API Integration", () => {
  ...
  test(
    "should handle real AI conversation with OpenRouter",
    { tag: "@expensive" },
    async ({ page }) => {
      const userMessage =
      ...
      await expect(page.locator(".ai-chat-message-assistant")).toBeVisible({
        timeout: 10000,
      });
  ...
  test(
    "should maintain chat history in real conversation",
    { tag: "@expensive" },
    async ({ page }) => {
